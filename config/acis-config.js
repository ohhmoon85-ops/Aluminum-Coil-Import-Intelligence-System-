...
window.ACIS_CONFIG = {
  ...
  // ---------------- 관세율 (v2.5) — 단일 진실 공급원 ----------------
  // index.html(DUTY_RATES) 과 api/signal.js 가 모두 이 값을 읽는다. 세율 변경 시 여기만 수정.
  // 출처: 관세법령정보포털(CLIP) 2026년 세율 직접 조회. FCN1 = 한·중 FTA 협정세율.
  // 세율은 FTA 이행 스케줄에 따라 매년 1.1. 변동 가능.
  //
  // ※ HSK 1000/9000 구분 기준은 '비합금/합금'이 아니라 알루미늄 순도 99.99%.
  //   당사 1050(99.5%)·1235(99.35%)는 99.99% 미달 → 전부 9000 라인 (합금 여부와 세율 무관).
  // ※ fta 세율은 원산지증명서(C/O) 구비 시 적용. 미구비 시 basic(기본세율).
  // ※ 지용성·수용성 코팅품의 7607.19 분류는 도포·경화 코팅의 '뒷면 부착' 부인정에 근거
  //   (국내 관세평가분류원 결정문 + 미국 CBP HQ 966769). 품목분류 사전심사 진행 중이나
  //   어느 라인이든 리스크는 하방(7607.20 인정 시 1.6%로 절감)뿐이므로 7.2%가 보수적 전제.
  // ※ 코팅 종류(지용성/수용성)는 HSCODE 분류 기준이 아니므로 oil/water 동일 매핑.
  duty: {
    raw:   { hscode: '7607.11-9000', fta: 7.2, basic: 8.0 },  // 생알미늄 호일 (DOS 도포품 동일 라인)
    oil:   { hscode: '7607.19-9000', fta: 7.2, basic: 8.0 },  // 지용성 코팅품
    water: { hscode: '7607.19-9000', fta: 7.2, basic: 8.0 },  // 수용성 코팅품
  },
